...
function Profile() {
  ...
  const onUpload = async () => {
      ...
      try {
          ...
          toast.success('Fotografia de profil a fost salvată.')
  ...
  const validateForm = () => {
    ...
    if (!oldPassword) {
      tempErrors.old = 'Introduceți parola actuală.'
      isValid = false
    }
    ...
    if (newPassword !== confirmPassword) {
      tempErrors.confirm = 'Parolele noi nu se potrivesc.'
      isValid = false
    } else if (!confirmPassword) {
      tempErrors.confirm = 'Confirmați noua parolă.'
      isValid = false
    }
  ...
  const onPasswordSubmit = async (e) => {
    ...
    try {
      ...
      const message = (error.response?.data?.message) || error.message || 'Eroare la actualizarea parolei.'
      
      if (message.toLowerCase().includes('veche') || message.toLowerCase().includes('actual')) {
    ...
    <div className="w-full flex flex-col items-center px-4 py-12 animate-in fade-in zoom-in duration-500">
      ...
      <div className="w-full max-w-4xl flex items-center justify-between mb-8">
        <Link to="/dashboard" className="group flex items-center gap-3 bg-white/5 hover:bg-white/10 text-white px-5 py-3 rounded-2xl transition-all border border-white/5">
          ...
          <span className="font-bold text-sm uppercase tracking-wider">Înapoi la Dashboard</span>
        </Link>
        <div className="flex items-center gap-2">
            <div className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse"></div>
            <span className="text-emerald-400 text-xs font-black uppercase tracking-widest">Sesiune Validă</span>
        </div>
      </div>
      ...
      <div className="w-full max-w-4xl bg-slate-900/60 backdrop-blur-2xl border border-white/10 rounded-[3rem] shadow-[0_0_60px_rgba(0,0,0,0.5)] overflow-hidden relative ring-1 ring-white/5">
        ...
        <div className="px-8 pb-12 md:px-16">
          ...
          <div className="relative flex justify-center -mt-24 mb-8">
            <div className="relative group">
                ...
                <div className="relative w-40 h-40 rounded-full border-4 border-slate-900 bg-slate-800 overflow-hidden shadow-2xl">
                    {preview ? (
                    <img src={preview} alt="Profil" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
                    ) : (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-slate-950/40 border border-white/5 p-6 rounded-3xl flex items-center gap-5 hover:border-blue-500/30 transition-all group">
              ...
              <div className="flex flex-col">
                <span className="text-[10px] text-slate-400 uppercase font-black tracking-widest mb-1">ID Utilizator</span>
                <span className="text-sm font-bold text-white tracking-widest bg-white/5 px-3 py-1 rounded-lg border border-white/5">{displayId}</span>
              </div>
            </div>
          ...
          <div className="mt-8 bg-slate-950/40 border border-white/5 p-6 md:p-8 rounded-3xl hover:border-blue-500/30 transition-all">
            <h2 className="text-xl font-black text-white mb-6 flex items-center gap-3 uppercase">
            ...
            <form onSubmit={onPasswordSubmit} className="space-y-6" noValidate>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      placeholder="Rescrieți parola nouă..."
              ...
              <div className="pt-2">
                ...
                >
                  <FaSave /> {isPasswordLoading ? 'Se procesează...' : 'Actualizează Parola'}
                </button>
              </div>
            </form>
          </div>
